...
+#directories+ and +#files+ is broken at the moment as we need to make the parser use
+Parser#build_group+ for only those two commands.

== TODO list

* object oriented: song commands and dir commands in MPD::Song, MPD::Directory.
* Make stickers a mixin for Playlist, Song, Directory...
* Namespace queue
* Playlist#rename -> Playlist#name=
* Figure out a way to make passing the mpd instance to MPD::Playlist.new unnecessary.
* Make parsing per-command (because playlist is an integer in the :status command and
  a string in :listplaylists)